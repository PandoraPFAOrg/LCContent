...
private:
  ...
  /**
   *  @brief  Get generic distance between hits in parent cluster candidate and those in nearby (typically overlapping)
   * daughter cluster candidate. For each layer in parent cluster evaluate distance to hits in adjacent layers in
   * daughter cluster.
   *
   ...
   */
  pandora::StatusCode GetGenericDistanceBetweenClusters(const pandora::Cluster* const pParentCluster,
   ...
   *  @return boolean
   */
  bool IsClusterFragment(const pandora::Cluster* const pParentCluster,
  ...
  unsigned int
      m_nAdjacentLayersToExamine; ///< Number of adjacent layers to examine when calculating cluster generic distance
  ...
  float m_maxHelixPathlengthToDaughter; ///< Max path length from helix calorimeter intersection to daughter cluster z
                                        ///< coordinate
  unsigned int
      m_helixDistanceNLayers; ///< Layers to examine to calculate distance from helix to hits in daughter cluster
  unsigned int m_helixDistanceMaxOccupiedLayers; ///< Max number of occupied layers to consider in helix-cluster
