...
private:
  ...
  /**
   *  @brief  Prepare clusters for the cone based merging algorithm, applying pre-selection cuts and performing a mip
   * fit to candidate parent clusters.
   *
   ...
   */
  pandora::StatusCode PrepareClusters(pandora::ClusterVector& daughterVector,
                                      ClusterFitResultMap& parentFitResultMap) const;

  /**
   *  @brief  Get the fraction of hits in a daughter candidate cluster that are contained in a cluster defined by a mip
   * fit to the parent candidate cluster
   *
  ...
  float m_maxInnerLayerSeparationNoTrack; ///< Max distance between parent/daughter inner centroids when parent has no
                                          ///< associated tracks
