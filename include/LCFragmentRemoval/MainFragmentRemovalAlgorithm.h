...
/**
 *  @brief  ChargedClusterContact class, describing the interactions and proximity between parent and daughter candidate
 * clusters
 */
...
private:
  ...
  float m_closestDistanceToHelix; ///< Closest distance between daughter cluster and helix fits to parent associated
                                  ///< tracks
};
...
private:
   ...
   */
  float GetRequiredEvidenceForMerge(const pandora::Cluster* const pDaughterCluster,
                                    const ChargedClusterContact& chargedClusterContact,
                                    const unsigned int correctionLayer, const float globalDeltaChi2);

  /**
   *  @brief  Get the cluster correction layer. Working from innermost to outermost layer, the correction layer is that
   * in which: i)  the number of hit-layers passes m_correctionLayerNHitLayers, or ii) the total calo hit hadronic
   * energy passes (m_correctionLayerEnergyFraction * cluster energy)
   *
   *  @param  pDaughterCluster address of the daughter cluster candidate
   *
...
inline float ChargedClusterContact::GetParentTrackEnergy() const { return m_parentTrackEnergy; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline float ChargedClusterContact::GetConeFraction2() const { return m_coneFraction2; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline float ChargedClusterContact::GetConeFraction3() const { return m_coneFraction3; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline float ChargedClusterContact::GetMeanDistanceToHelix() const { return m_meanDistanceToHelix; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline float ChargedClusterContact::GetClosestDistanceToHelix() const { return m_closestDistanceToHelix; }
