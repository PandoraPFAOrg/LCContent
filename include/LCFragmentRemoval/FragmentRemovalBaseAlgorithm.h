...
protected:
     ...
     */
    virtual void GetRelevantClusterList(const pandora::ClusterList &inputClusterList, pandora::ClusterList &clusterList) const;
     ...
     *  @return boolean
     */
    virtual bool IsCandidateParent(const pandora::Cluster *const pDaughterCluster, const pandora::Cluster *const pParentCluster) const = 0;
     ...
     */
    virtual pandora::StatusCode GetClusterMergingCandidates(const ContactMap &contactMap, const pandora::Cluster *&pBestParentCluster,
...
private:
     ...
     */
    pandora::StatusCode GetClusterContactMap(bool &isFirstPass, const pandora::ClusterSet &affectedClusters, ClusterContactCache &contactCache,
     ...
     */
    void UpdateClusterContacts(const pandora::Cluster *const pDaughterCluster, const pandora::ClusterVector &changedClusters,
     ...
     */
    pandora::StatusCode GetAffectedClusters(const ContactMap &contactMap, const pandora::Cluster *const pBestParentCluster,
...
inline FragmentRemovalBaseAlgorithm<CONTACT>::FragmentRemovalBaseAlgorithm() :
    m_contactParameters(),
    m_nMaxPasses(std::numeric_limits<unsigned int>::max()),
    m_minDaughterCaloHits(0),
    m_minDaughterHadronicEnergy(0.f),
    m_contactCutMaxDistance(0.f)
{
}
...
template <typename CONTACT>
inline pandora::StatusCode FragmentRemovalBaseAlgorithm<CONTACT>::PostMergeAction(const pandora::Cluster *const /*pBestParentCluster*/)
{
...
template <typename CONTACT>
inline bool FragmentRemovalBaseAlgorithm<CONTACT>::CouldPassClusterContactCuts(const pandora::Cluster *const pDaughterCluster,
    const ClusterBoundingBox &daughterBoundingBox, const pandora::Cluster *const pParentCluster, ClusterContactCache &contactCache) const
{
...
pandora::StatusCode FragmentRemovalBaseAlgorithm<CONTACT>::Run()
{
    ...
    while ((nPasses++ < m_nMaxPasses) && shouldRecalculate)
    {
        ...
        PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, this->GetClusterContactMap(isFirstPass, affectedClusters, contactCache,
            contactMap));

        PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, this->GetClusterMergingCandidates(contactMap, pBestParentCluster,
            pBestDaughterCluster));

        if ((NULL != pBestParentCluster) && (NULL != pBestDaughterCluster))
        {
            PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, this->GetAffectedClusters(contactMap, pBestParentCluster,
...
template <typename CONTACT>
pandora::StatusCode FragmentRemovalBaseAlgorithm<CONTACT>::GetClusterContactMap(bool &isFirstPass, const pandora::ClusterSet &affectedClusters,
    ClusterContactCache &contactCache, ContactMap &contactMap) const
{
    ...
    for (const pandora::Cluster *const pDaughterCluster : clusterList)
    {
        ...
        if (!isFullRebuild)
        {
            this->UpdateClusterContacts(pDaughterCluster, changedClusters, changedClusterSet, clusterToIndex, contactCache, contactMap);
            continue;
        }
...
void FragmentRemovalBaseAlgorithm<CONTACT>::UpdateClusterContacts(const pandora::Cluster *const pDaughterCluster,
    const pandora::ClusterVector &changedClusters, const pandora::ClusterSet &changedClusterSet, const ClusterToIndexMap &clusterToIndex,
    ClusterContactCache &contactCache, ContactMap &contactMap) const
{
...
template <typename CONTACT>
pandora::StatusCode FragmentRemovalBaseAlgorithm<CONTACT>::GetAffectedClusters(const ContactMap &contactMap,
    const pandora::Cluster *const pBestParentCluster, const pandora::Cluster *const pBestDaughterCluster,
    pandora::ClusterSet &affectedClusters) const
{
    ...
    for (const auto &mapEntry : contactMap)
    {
        ...
        // Also store addresses of all clusters that contained either the parent or daughter clusters in their own contact vectors
        for (const CONTACT &contact : mapEntry.second)
        {
