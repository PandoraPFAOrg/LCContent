...
/**
 *  @brief  ClusterContact class, describing the interactions and proximity between parent and daughter candidate
 * clusters
 */
...
public:
   ...
   */
  static float GetFractionOfCloseHits(const pandora::Cluster* const pClusterI, const pandora::Cluster* const pClusterJ,
                                      const float distanceThreshold);

  /**
   *  @brief  Get the fraction of calo hits in cluster I that lie within a cone along the direction of cluster J.
   *          If cluster J has an associated track, it's projected position and direction at the calorimeter will be
   * used to define cone, otherwise cone will be radial and its axis will pass through centroid of cluster J at its
   * showerstart layer.
   *
   *  @param  pandora the associated pandora instance
   *  @param  pClusterI address of the cluster for which the fraction is calculated
   ...
   */
  static float GetFractionOfHitsInCone(const pandora::Pandora& pandora, const pandora::Cluster* const pClusterI,
                                       const pandora::Cluster* const pClusterJ, const float coneCosineHalfAngle);
   ...
   *  @return The fraction of calo hits in the cone
   */
  static float GetFractionOfHitsInCone(const pandora::Cluster* const pCluster, const pandora::Track* const pTrack,
   ...
   */
  static unsigned int GetNLayersCrossed(const pandora::Pandora& pandora, const pandora::Helix& helix,
                                        const float zStart, const float zEnd, const unsigned int nSamplingPoints = 100);
   ...
   *  @param  closestDistanceToHit to receive the closest distance between the helix and a hit in the specified range of
   * the cluster
   *  @param  meanDistanceToHits to receive the mean distance between the helix and hits in the specified range of the
   * cluster
   */
  static pandora::StatusCode GetClusterHelixDistance(const pandora::Cluster* const pCluster,
                                                     const pandora::Helix& helix, const unsigned int startLayer,
                                                     const unsigned int endLayer, const unsigned int maxOccupiedLayers,
                                                     float& closestDistanceToHit, float& meanDistanceToHits);

  /**
   *  @brief  Get the number of contact layers for two clusters and also the ratio of the number of contact layers to
   * overlap layers
   *
   ...
   */
  static pandora::StatusCode GetClusterContactDetails(const pandora::Cluster* const pClusterI,
   ...
   */
  static float GetEMEnergyWeightedLayerSeparation(const pandora::Cluster* const pClusterI,
                                                  const pandora::Cluster* const pClusterJ);
...
inline const pandora::Cluster* ClusterContact::GetDaughterCluster() const { return m_pDaughterCluster; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline const pandora::Cluster* ClusterContact::GetParentCluster() const { return m_pParentCluster; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline unsigned int ClusterContact::GetNContactLayers() const { return m_nContactLayers; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline float ClusterContact::GetContactFraction() const { return m_contactFraction; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline float ClusterContact::GetDistanceToClosestHit() const { return m_distanceToClosestHit; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline float ClusterContact::GetCloseHitFraction1() const { return m_closeHitFraction1; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline float ClusterContact::GetCloseHitFraction2() const { return m_closeHitFraction2; }

//------------------------------------------------------------------------------------------------------------------------------------------

inline float ClusterContact::GetConeFraction1() const { return m_coneFraction1; }
