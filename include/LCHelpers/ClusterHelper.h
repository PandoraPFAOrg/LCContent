...
public:
   ...
   */
  static pandora::StatusCode GetFitResultsClosestApproach(const pandora::ClusterFitResult& lhs,
                                                          const pandora::ClusterFitResult& rhs, float& closestApproach);
   ...
   */
  static float GetDistanceToClosestHit(const pandora::ClusterFitResult& clusterFitResult,
                                       const pandora::Cluster* const pCluster, const unsigned int startLayer,
   ...
   */
  static float GetDistanceToClosestHit(const pandora::Cluster* const pClusterI,
                                       const pandora::Cluster* const pClusterJ);

  /**
   *  @brief  Get closest distance of approach between projected cluster fit result and layer centroid position of a
   * second cluster
   *
   *  @param  clusterFitResult the fit result to the first cluster
   *  @param  pCluster address of the second cluster
   ...
   *  @return the closest distance of approach
   */
  static float GetDistanceToClosestCentroid(const pandora::ClusterFitResult& clusterFitResult,
                                            const pandora::Cluster* const pCluster, const unsigned int startLayer,
                                            const unsigned int endLayer);
   ...
   */
  static pandora::StatusCode GetDistanceToClosestCentroid(const pandora::Cluster* const pClusterI,
                                                          const pandora::Cluster* const pClusterJ,
   ...
   */
  static pandora::StatusCode GetClosestIntraLayerDistance(const pandora::Cluster* const pClusterI,
                                                          const pandora::Cluster* const pClusterJ,
                                                          float& intraLayerDistance);

  /**
   *  @brief  Get the distance of closest approach between the projected track direction at calorimeter and the hits
   * within a cluster. Note that only a specified number of layers are examined.
   *
  ...
  static pandora::StatusCode GetTrackClusterDistance(const T* const pT, const pandora::Cluster* const pCluster,
                                                     const unsigned int maxSearchLayer, const float parallelDistanceCut,
                                                     const float minTrackClusterCosAngle, float& trackClusterDistance);

  /**
   *  @brief  Whether a cluster can be merged with another. Uses simple suggested criteria, including cluster photon id
   * flag and supplied cuts on cluster mip fraction and all hits fit rms.
   *
   ...
   *  @return boolean
   */
  static bool CanMergeCluster(const pandora::Pandora& pandora, const pandora::Cluster* const pCluster,
  ...
  /**
   *  @brief  Whether a linear fit to a cluster crosses a registered gap region. Only the region between the startlayer
   * and endlayer is considered in the fit and in the comparison with registered gap regions.
   *
   *  @param  pandora the associated pandora instance
   *  @param  pCluster address of the cluster
   ...
   *  @return boolean
   */
  static bool DoesClusterCrossGapRegion(const pandora::Pandora& pandora, const pandora::Cluster* const pCluster,
                                        const unsigned int startLayer, const unsigned int endLayer,
                                        const unsigned int nSamplingPoints = 50);

  /**
   *  @brief  Whether a linear fit crosses a registered gap region. The fit will be propagated through the specified
   * distance from its closest approach to the startPosition. Within this propagation, the fit will be sampled a
   * specified number of times and the resulting position compared with registered gap regions.
   *
   *  @param  pandora the associated pandora instance
  ...
  static bool DoesFitCrossGapRegion(const pandora::Pandora& pandora, const pandora::ClusterFitResult& clusterFitResult,
                                    const pandora::CartesianVector& startPosition, const float propagationDistance,
                                    const unsigned int nSamplingPoints = 50);
   ...
   *  @return boolean
   */
  static bool IsClusterLeavingDetector(const pandora::Cluster* const pCluster,
