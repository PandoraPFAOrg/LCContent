...
ConeClusteringAlgorithm::ConeClusteringAlgorithm()
    : m_clusterSeedStrategy(2), m_shouldUseOnlyECalHits(false), m_shouldUseIsolatedHits(false),
      ...
      m_minClusterDirProjection(-10.f), m_trackPathWidth(2.f), m_maxTrackSeedSeparation2(250.f * 250.f),
      m_maxLayersToTrackSeed(3), m_maxLayersToTrackLikeHit(3), m_nLayersSpannedForFit(6),
      m_nLayersSpannedForApproxFit(10), m_nLayersToFit(8), m_nLayersToFitLowMipCut(0.5f),
      m_nLayersToFitLowMipMultiplier(2), m_fitSuccessDotProductCut1(0.75f), m_fitSuccessChi2Cut1(5.0f),
      m_fitSuccessDotProductCut2(0.50f), m_fitSuccessChi2Cut2(2.5f), m_mipTrackChi2Cut(2.5f), m_firstLayer(1) {}
  ...
  for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(), iterEnd = orderedCaloHitList.end();
    ...
    for (CaloHitList::const_iterator hitIter = iter->second->begin(), hitIterEnd = iter->second->end();
      ...
      if ((m_shouldUseIsolatedHits || !pCaloHit->IsIsolated()) &&
          ...
          (!m_shouldExcludeBIBHits || !pCaloHit->IsPossibleBIB()) &&
    ...
    PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=,
        this->FindHitsInPreviousLayers(pseudoLayer, relevantCaloHits, clusterFitResultMap, clusterVector));
    PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=,
        this->FindHitsInSameLayer(pseudoLayer, relevantCaloHits, clusterFitResultMap, clusterVector));
  }
...
StatusCode ConeClusteringAlgorithm::InitializeKDTrees(const TrackList* const pTrackList,
...
StatusCode ConeClusteringAlgorithm::SeedClustersWithTracks(const TrackList* const pTrackList,
                                                           ClusterVector& clusterVector) {
  if (0 == m_clusterSeedStrategy)
    return STATUS_CODE_SUCCESS;
...
StatusCode ConeClusteringAlgorithm::GetCurrentClusterFitResults(const ClusterVector& clusterVector,
                                                                ClusterFitResultMap& clusterFitResultMap) const {
  if (!clusterFitResultMap.empty())
    return STATUS_CODE_INVALID_PARAMETER;

  for (ClusterVector::const_iterator iter = clusterVector.begin(), iterEnd = clusterVector.end(); iter != iterEnd;
       ++iter) {
...
// inprogress
StatusCode ConeClusteringAlgorithm::FindHitsInPreviousLayers(unsigned int pseudoLayer,
                                                             const CaloHitVector& relevantCaloHits,
                                                             const ClusterFitResultMap& clusterFitResultMap,
                                                             ClusterVector& /*clusterVector*/) {
  ...
  for (const CaloHit* const pCaloHit : relevantCaloHits) {
    ...
    const float additionalPadWidths =
    ...
    const float largestAllowedDistanceForSearch =
        std::max(maxTrackSeedSeparation, m_maxClusterDirProjection + additionalPadWidths);

    const Cluster* pBestCluster = nullptr;
    float bestClusterEnergy(0.f);
    float smallestGenericDistance(m_genericDistanceCut);
    const unsigned int layersToStepBack(
        (PandoraContentApi::GetGeometry(*this)->GetHitTypeGranularity(pCaloHit->GetHitType()) <= FINE)
            ? m_layersToStepBackFine
            : m_layersToStepBackCoarse);

    // Associate with existing clusters in stepBack layers. If stepBackLayer == pseudoLayer, will examine track
    // projections
    for (unsigned int stepBackLayer = 1; (stepBackLayer <= layersToStepBack) && (stepBackLayer <= pseudoLayer);
         ++stepBackLayer) {
      ...
      KDTreeCube searchRegionTks = build_3d_kd_search_region(
          pCaloHit, largestAllowedDistanceForSearch, largestAllowedDistanceForSearch, largestAllowedDistanceForSearch);
      ...
      KDTreeTesseract searchRegionHits =
          build_4d_kd_search_region(pCaloHit, largestAllowedDistanceForSearch, largestAllowedDistanceForSearch,
                                    largestAllowedDistanceForSearch, searchLayer);
      ...
      // Instead of using the full cluster list we use only those clusters that are found to be nearby according to the
      // KD-tree
      // ---- This can be optimized further for sure. (for instance having a match by KD-tree qualifies a ton of the
      // loops later See if hit should be associated with any existing clusters
      for (const Cluster* const pCluster : nearbyClusterList) {
        ...
        PANDORA_RETURN_RESULT_IF_AND_IF(
            STATUS_CODE_SUCCESS, STATUS_CODE_UNCHANGED, !=,
            this->GetGenericDistanceToHit(pCluster, pCaloHit, searchLayer, clusterFitResultMap, genericDistance));
      ...
      if ((0 == m_clusterFormationStrategy) && (nullptr != pBestCluster)) {
        PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                                 PandoraContentApi::AddToCluster(*this, pBestCluster, pCaloHit));
      ...
      for (auto iter = available_hits_in_layer.begin(); iter != available_hits_in_layer.end();) {
        ...
        if (track_assc_cache != hitsToTracksLocal.end()) {
          ...
          KDTreeCube searchRegionTks =
              build_3d_kd_search_region(pCaloHit, track_search_width, track_search_width, track_search_width);
          m_tracksKdTree.search(searchRegionTks, found_tracks);
          for (auto& track : found_tracks) {
        ...
        if (hits_assc_cache != hitsToHitsLocal.end()) {
          ...
          KDTreeTesseract searchRegionHits =
              build_4d_kd_search_region(pCaloHit, hit_search_width, hit_search_width, hit_search_width, pseudoLayer);
          m_hitsKdTree.search(searchRegionHits, found_hits);
          for (auto& hit : found_hits) {
        ...
        for (ClusterList::iterator clusterIter = nearbyClusterList.begin(), clusterIterEnd = nearbyClusterList.end();
          ...
          PANDORA_RETURN_RESULT_IF_AND_IF(
              STATUS_CODE_SUCCESS, STATUS_CODE_UNCHANGED, !=,
              this->GetGenericDistanceToHit(pCluster, pCaloHit, pseudoLayer, clusterFitResultMap, genericDistance));
...
StatusCode ConeClusteringAlgorithm::GetGenericDistanceToHit(const Cluster* const pCluster,
                                                            const CaloHit* const pCaloHit,
                                                            const unsigned int searchLayer,
                                                            const ClusterFitResultMap& clusterFitResultMap,
  ...
  const ClusterFitResult* const clusterFitResult((clusterFitResultMap.end() != fitResultIter) ? &(fitResultIter->second)
                                                                                              : nullptr);
  const CartesianVector& clusterDirection((clusterFitResult != nullptr && clusterFitResult->IsFitSuccessful())
                                              ? clusterFitResult->GetDirection()
  ...
  const bool followInitialDirection(m_shouldFollowInitialDirection && pCluster->IsTrackSeeded() &&
                                    (searchLayer > m_trackSeedCutOffLayer));
  ...
  if (!useTrackSeed || (searchLayer > m_trackSeedCutOffLayer)) {
    ...
    StatusCode initialStatusCode = this->GetConeApproachDistanceToHit(
        pCaloHit, pClusterCaloHitList, pCluster->GetInitialDirection(), initialDirectionDistance);
    ...
    if (clusterFitResult != nullptr && clusterFitResult->IsFitSuccessful()) {
      StatusCode currentStatusCode = this->GetConeApproachDistanceToHit(
          pCaloHit, pClusterCaloHitList, clusterFitResult->GetDirection(), currentDirectionDistance);
  ...
  const float smallestDistance(
      std::min(trackSeedDistance, std::min(initialDirectionDistance, currentDirectionDistance)));
...
StatusCode ConeClusteringAlgorithm::GetDistanceToHitInSameLayer(const CaloHit* const pCaloHit,
                                                                const CaloHitList* const pCaloHitList,
...
StatusCode ConeClusteringAlgorithm::GetConeApproachDistanceToHit(const CaloHit* const pCaloHit,
                                                                 const CaloHitList* const pCaloHitList,
  ...
  for (CaloHitList::const_iterator iter = pCaloHitList->begin(), iterEnd = pCaloHitList->end(); iter != iterEnd;
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_UNCHANGED, !=,
                                    this->GetConeApproachDistanceToHit(pCaloHit, pHitInCluster->GetPositionVector(),
                                                                       clusterDirection, hitDistance));
...
StatusCode ConeClusteringAlgorithm::GetConeApproachDistanceToHit(const CaloHit* const pCaloHit,
                                                                 const CartesianVector& clusterPosition,
                                                                 const CartesianVector& clusterDirection,
                                                                 float& distance) const {
  const CartesianVector& hitPosition(pCaloHit->GetPositionVector());
  ...
  if ((dAlong < m_maxClusterDirProjection) && (dAlong > m_minClusterDirProjection)) {
    const float dCut((PandoraContentApi::GetGeometry(*this)->GetHitTypeGranularity(pCaloHit->GetHitType()) <= FINE)
                         ? (std::fabs(dAlong) * m_tanConeAngleFine) +
                               (m_additionalPadWidthsFine * pCaloHit->GetCellLengthScale())
                         : (std::fabs(dAlong) * m_tanConeAngleCoarse) +
                               (m_additionalPadWidthsCoarse * pCaloHit->GetCellLengthScale()));
...
StatusCode ConeClusteringAlgorithm::GetDistanceToTrackSeed(const Cluster* const pCluster, const CaloHit* const pCaloHit,
  ...
  for (int iLayer = startLayer; iLayer < searchLayerInt; ++iLayer) {
    ...
    for (CaloHitList::const_iterator iter = listIter->second->begin(), iterEnd = listIter->second->end();
         iter != iterEnd; ++iter) {
...
StatusCode ConeClusteringAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ClusterSeedStrategy", m_clusterSeedStrategy));

  // High level clustering parameters
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ShouldUseOnlyECalHits", m_shouldUseOnlyECalHits));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "LayersToStepBackCoarse", m_layersToStepBackCoarse));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ClusterFormationStrategy", m_clusterFormationStrategy));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "GenericDistanceCut", m_genericDistanceCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinHitTrackCosAngle", m_minHitTrackCosAngle));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinHitClusterCosAngle", m_minHitClusterCosAngle));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ShouldUseTrackSeed", m_shouldUseTrackSeed));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "TrackSeedCutOffLayer", m_trackSeedCutOffLayer));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ShouldFollowInitialDirection", m_shouldFollowInitialDirection));

  // Same layer distance parameters
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "SameLayerPadWidthsFine", m_sameLayerPadWidthsFine));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "SameLayerPadWidthsCoarse", m_sameLayerPadWidthsCoarse));

  // Cone approach distance parameters
  float coneApproachMaxSeparation = std::sqrt(m_coneApproachMaxSeparation2);
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ConeApproachMaxSeparation", coneApproachMaxSeparation));
  m_coneApproachMaxSeparation2 = coneApproachMaxSeparation * coneApproachMaxSeparation;

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "TanConeAngleFine", m_tanConeAngleFine));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "TanConeAngleCoarse", m_tanConeAngleCoarse));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "AdditionalPadWidthsFine", m_additionalPadWidthsFine));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "AdditionalPadWidthsCoarse", m_additionalPadWidthsCoarse));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxClusterDirProjection", m_maxClusterDirProjection));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MinClusterDirProjection", m_minClusterDirProjection));

  // Track seed distance parameters
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "TrackPathWidth", m_trackPathWidth));

  float maxTrackSeedSeparation = std::sqrt(m_maxTrackSeedSeparation2);
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxTrackSeedSeparation", maxTrackSeedSeparation));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxLayersToTrackSeed", m_maxLayersToTrackSeed));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxLayersToTrackLikeHit", m_maxLayersToTrackLikeHit));

  // Cluster current direction and mip track parameters
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NLayersSpannedForFit", m_nLayersSpannedForFit));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "NLayersSpannedForApproxFit", m_nLayersSpannedForApproxFit));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NLayersToFit", m_nLayersToFit));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "NLayersToFitLowMipCut", m_nLayersToFitLowMipCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "NLayersToFitLowMipMultiplier", m_nLayersToFitLowMipMultiplier));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "FitSuccessDotProductCut1", m_fitSuccessDotProductCut1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "FitSuccessChi2Cut1", m_fitSuccessChi2Cut1));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "FitSuccessDotProductCut2", m_fitSuccessDotProductCut2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "FitSuccessChi2Cut2", m_fitSuccessChi2Cut2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MipTrackChi2Cut", m_mipTrackChi2Cut));
