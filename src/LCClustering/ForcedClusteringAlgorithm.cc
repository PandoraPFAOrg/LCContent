...
StatusCode ForcedClusteringAlgorithm::Run() {
  ...
  for (TrackList::const_iterator iter = pTrackList->begin(), iterEnd = pTrackList->end(); iter != iterEnd; ++iter) {
    ...
    for (CaloHitList::const_iterator hitIter = pCaloHitList->begin(), hitIterEnd = pCaloHitList->end();
      ...
      if ((m_shouldClusterIsolatedHits || !pCaloHit->IsIsolated()) &&
          (!m_shouldExcludeBIBHits || !pCaloHit->IsPossibleBIB()) && PandoraContentApi::IsAvailable(*this, pCaloHit)) {
        CartesianVector helixSeparation(0.f, 0.f, 0.f);
        PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                                 helix.GetDistanceToPoint(pCaloHit->GetPositionVector(), helixSeparation));

        trackDistanceInfoVector.push_back(
            TrackDistanceInfo(pCaloHit, pCluster, trackEnergy, helixSeparation.GetMagnitude()));
      }
    }
  }

  std::sort(trackDistanceInfoVector.begin(), trackDistanceInfoVector.end(),
            ForcedClusteringAlgorithm::SortByDistanceToTrack);

  // Work along ordered list of calo hits, adding to the clusters until each cluster energy matches associated track
  // energy.
  ...
  if (m_shouldRunStandardClusteringAlgorithm) {
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             PandoraContentApi::RunDaughterAlgorithm(*this, m_standardClusteringAlgorithmName));
  } else {
    ...
    for (CaloHitList::const_iterator iter = pCaloHitList->begin(), iterEnd = pCaloHitList->end(); iter != iterEnd;
      ...
      if ((m_shouldClusterIsolatedHits || !(*iter)->IsIsolated()) &&
          (!m_shouldExcludeBIBHits || !(*iter)->IsPossibleBIB()) && PandoraContentApi::IsAvailable(*this, *iter))
    ...
    if (!remnantCaloHitList.empty()) {
      ...
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               PandoraContentApi::Cluster::Create(*this, remnantParameters, pRemnantCluster));
    }
  }
  ...
  if (m_shouldAssociateIsolatedHits) {
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                             PandoraContentApi::RunDaughterAlgorithm(*this, m_isolatedHitAssociationAlgorithmName));
  }
...
StatusCode ForcedClusteringAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      XmlHelper::ReadValue(xmlHandle, "ShouldRunStandardClusteringAlgorithm", m_shouldRunStandardClusteringAlgorithm));

  if (m_shouldRunStandardClusteringAlgorithm) {
    PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=,
        XmlHelper::ProcessAlgorithm(*this, xmlHandle, "StandardClustering", m_standardClusteringAlgorithmName));
  }
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ShouldAssociateIsolatedHits", m_shouldAssociateIsolatedHits));

  if (m_shouldAssociateIsolatedHits) {
    PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=,
        XmlHelper::ProcessAlgorithm(*this, xmlHandle, "IsolatedHitAssociation", m_isolatedHitAssociationAlgorithmName));
  }
