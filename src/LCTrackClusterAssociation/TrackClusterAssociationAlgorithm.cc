  ...
  for (const Track* const pTrack : trackVector) {
    ...
    for (auto const& trackState : trackStates) {
      ...
      for (unsigned iPseudoLayer = 0; iPseudoLayer <= m_maxSearchLayer; ++iPseudoLayer) {
        ...
        if (cached_result.first != tracks_to_hits.end()) {
          ...
          KDTreeTesseract searchRegionHits = build_4d_kd_search_region(
              trackPosition, m_parallelDistanceCut, m_parallelDistanceCut, m_parallelDistanceCut, iPseudoLayer);
      ...
      for (const Cluster* const pCluster : nearbyClusterList) {
        ...
        if (STATUS_CODE_SUCCESS != lc_content::ClusterHelper::GetTrackClusterDistance(
                                       &trackState, pCluster, m_maxSearchLayer, m_parallelDistanceCut,
        ...
        if (clusterHadronicEnergy > m_lowEnergyCut) {
          ...
          if ((trackClusterDistance < minLowEnergyDistance) ||
              ((trackClusterDistance == minLowEnergyDistance) && (energyDifference < minLowEnergyDifference))) {
    ...
    if (nullptr != pMatchedCluster) {
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               PandoraContentApi::AddTrackClusterAssociation(*this, pTrack, pMatchedCluster));
    }
  } // for all tracks
...
StatusCode TrackClusterAssociationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "LowEnergyCut", m_lowEnergyCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxTrackClusterDistance", m_maxTrackClusterDistance));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxSearchLayer", m_maxSearchLayer));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ParallelDistanceCut", m_parallelDistanceCut));
