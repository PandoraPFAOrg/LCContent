...
NeutralFragmentRemovalAlgorithm::NeutralFragmentRemovalAlgorithm()
    : m_nMaxPasses(200), m_minDaughterCaloHits(5), m_minDaughterHadronicEnergy(0.025f), m_photonLikeMaxInnerLayer(10),
      m_photonLikeMinDCosR(0.5f), m_photonLikeMaxShowerStart(5.f), m_photonLikeMaxProfileDiscrepancy(0.75f),
      m_contactCutMaxDistance(500.f), m_contactCutNLayers(2), m_contactCutConeFraction1(0.5f),
...
StatusCode NeutralFragmentRemovalAlgorithm::Run() {
  ...
  while ((nPasses++ < m_nMaxPasses) && shouldRecalculate) {
    ...
    PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=,
        this->GetNeutralClusterContactMap(isFirstPass, affectedClusters, neutralClusterContactMap));

    PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=,
        this->GetClusterMergingCandidates(neutralClusterContactMap, pBestParentCluster, pBestDaughterCluster));

    if ((NULL != pBestParentCluster) && (NULL != pBestDaughterCluster)) {
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               this->GetAffectedClusters(neutralClusterContactMap, pBestParentCluster,
...
StatusCode
NeutralFragmentRemovalAlgorithm::GetNeutralClusterContactMap(bool& isFirstPass, const ClusterSet& affectedClusters,
  ...
  for (ClusterList::const_iterator iterI = pClusterList->begin(), iterIEnd = pClusterList->end(); iterI != iterIEnd;
    ...
    for (ClusterList::const_iterator iterJ = pClusterList->begin(), iterJEnd = pClusterList->end(); iterJ != iterJEnd;
      ...
      const NeutralClusterContact neutralClusterContact(this->GetPandora(), pDaughterCluster, pParentCluster,
                                                        m_contactParameters);
...
StatusCode
NeutralFragmentRemovalAlgorithm::GetClusterMergingCandidates(const NeutralClusterContactMap& neutralClusterContactMap,
...
StatusCode NeutralFragmentRemovalAlgorithm::GetAffectedClusters(
    const NeutralClusterContactMap& neutralClusterContactMap, const Cluster* const pBestParentCluster,
  ...
  for (NeutralClusterContactMap::const_iterator iterI = neutralClusterContactMap.begin(),
                                                iterIEnd = neutralClusterContactMap.end();
    ...
    if (iterI->first == pBestDaughterCluster) {
      for (NeutralClusterContactVector::const_iterator iterJ = iterI->second.begin(), iterJEnd = iterI->second.end();
