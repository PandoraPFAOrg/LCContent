...
MainFragmentRemovalAlgorithm::MainFragmentRemovalAlgorithm()
    : m_minDaughterCaloHits(5), m_minDaughterHadronicEnergy(0.025f), m_contactCutMaxDistance(750.f),
      m_contactCutNLayers(0), m_contactCutConeFraction1(0.25f), m_contactCutCloseHitFraction1(0.25f),
      m_contactCutCloseHitFraction2(0.15f), m_contactCutMeanDistanceToHelix(250.f),
      m_contactCutClosestDistanceToHelix(150.f), m_contactCutMaxHitDistance(250.f),
      m_contactCutMinDaughterInnerLayer(19), m_maxChi2(16.f), m_maxGlobalChi2(9.f), m_chi2Base(5.f),
      m_globalChi2Penalty(5.f), m_correctionLayerNHitLayers(3), m_correctionLayerEnergyFraction(0.25f),
      m_contactEvidenceNLayers1(10), m_contactEvidenceNLayers2(4), m_contactEvidenceNLayers3(1),
      m_contactEvidence1(2.f), m_contactEvidence2(1.f), m_contactEvidence3(0.5f), m_coneEvidenceFraction1(0.5f),
      m_coneEvidenceFineGranularityMultiplier(0.5f), m_closestTrackEvidence1(200.f), m_closestTrackEvidence1d(100.f),
      m_closestTrackEvidence2(50.f), m_closestTrackEvidence2d(20.f), m_meanTrackEvidence1(200.f),
      m_meanTrackEvidence1d(100.f), m_meanTrackEvidence2(50.f), m_meanTrackEvidence2d(50.f), m_distanceEvidence1(100.f),
      ...
      m_trackExtrapolationWeight(1.f), m_layerCorrectionLayerValue1(15), m_layerCorrectionLayerValue2(30),
      m_layerCorrectionLayerValue3(50), m_layerCorrection1(2.f), m_layerCorrection2(0.f), m_layerCorrection3(-1.f),
      m_layerCorrection4(-2.f), m_layerCorrectionLayerSpan(4), m_layerCorrectionMinInnerLayer(5),
      m_layerCorrection5(-2.f), m_leavingCorrection(5.f), m_energyCorrectionThreshold(3.f),
      m_lowEnergyCorrectionThreshold(1.5f), m_lowEnergyCorrectionNHitLayers1(6), m_lowEnergyCorrectionNHitLayers2(4),
      m_lowEnergyCorrectionNHitLayers3(29), m_lowEnergyCorrection1(-1.f), m_lowEnergyCorrection2(-1.f),
      m_lowEnergyCorrection3(-1.f), m_angularCorrectionOffset(0.75f), m_angularCorrectionConstant(-0.5f),
      m_angularCorrectionGradient(2.f), m_photonCorrectionEnergy1(2.f), m_photonCorrectionEnergy2(0.5f),
      m_photonCorrectionEnergy3(1.f), m_photonCorrectionShowerStart1(5.f), m_photonCorrectionShowerStart2(2.5f),
      m_photonCorrectionShowerDiscrepancy1(0.8f), m_photonCorrectionShowerDiscrepancy2(1.f), m_photonCorrection1(10.f),
      m_photonCorrection2(100.f), m_photonCorrection3(5.f), m_photonCorrection4(10.f), m_photonCorrection5(2.f),
      m_photonCorrection6(2.f), m_photonCorrection7(0.f), m_minRequiredEvidence(0.5f) {
...
StatusCode MainFragmentRemovalAlgorithm::Run() {
  ...
  while (shouldRecalculate) {
    ...
    PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=,
        this->GetChargedClusterContactMap(isFirstPass, affectedClusters, chargedClusterContactMap));

    PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=,
        this->GetClusterMergingCandidates(chargedClusterContactMap, pBestParentCluster, pBestDaughterCluster));

    if ((NULL != pBestParentCluster) && (NULL != pBestDaughterCluster)) {
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               this->GetAffectedClusters(chargedClusterContactMap, pBestParentCluster,
...
StatusCode
MainFragmentRemovalAlgorithm::GetChargedClusterContactMap(bool& isFirstPass, const ClusterSet& affectedClusters,
  ...
  for (ClusterList::const_iterator iterI = clusterList.begin(), iterIEnd = clusterList.end(); iterI != iterIEnd;
       ++iterI) {
    ...
    for (ClusterList::const_iterator iterJ = clusterList.begin(), iterJEnd = clusterList.end(); iterJ != iterJEnd;
      ...
      const ChargedClusterContact chargedClusterContact(this->GetPandora(), pDaughterCluster, pParentCluster,
                                                        m_contactParameters);
...
StatusCode
MainFragmentRemovalAlgorithm::GetClusterMergingCandidates(const ChargedClusterContactMap& chargedClusterContactMap,
  ...
  ClusterList clusterList;
  for (const auto& mapEntry : chargedClusterContactMap)
  ...
  for (const Cluster* const pDaughterCluster : clusterList) {
    ...
    for (const ChargedClusterContact& chargedClusterContact : contactVector) {
      ...
      const float requiredEvidence(this->GetRequiredEvidenceForMerge(pDaughterCluster, chargedClusterContact,
                                                                     daughterCorrectionLayer, globalDeltaChi2));
      ...
      if ((excessEvidence > highestExcessEvidence) ||
          ((excessEvidence == highestExcessEvidence) && (parentEnergy > highestEvidenceParentEnergy))) {
...
bool MainFragmentRemovalAlgorithm::PassesPreselection(const Cluster* const pDaughterCluster,
  ...
  // Check to see if merging parent and daughter clusters would improve track-cluster compatibility
  for (ChargedClusterContactVector::const_iterator iter = chargedClusterContactVector.begin(),
                                                   iterEnd = chargedClusterContactVector.end();
       iter != iterEnd; ++iter) {
    ChargedClusterContact chargedClusterContact = *iter;
    const float parentTrackEnergy(chargedClusterContact.GetParentTrackEnergy());
    const float parentClusterEnergy(
        chargedClusterContact.GetParentCluster()->GetTrackComparisonEnergy(this->GetPandora()));

    const float oldChi(
        ReclusterHelper::GetTrackClusterCompatibility(this->GetPandora(), parentClusterEnergy, parentTrackEnergy));
    const float newChi(ReclusterHelper::GetTrackClusterCompatibility(
        this->GetPandora(), daughterClusterEnergy + parentClusterEnergy, parentTrackEnergy));
  ...
  const float oldChiTotal(
      ReclusterHelper::GetTrackClusterCompatibility(this->GetPandora(), totalClusterEnergy, totalTrackEnergy));
  const float newChiTotal(ReclusterHelper::GetTrackClusterCompatibility(
      this->GetPandora(), daughterClusterEnergy + totalClusterEnergy, totalTrackEnergy));
...
float MainFragmentRemovalAlgorithm::GetTotalEvidenceForMerge(const ChargedClusterContact& chargedClusterContact) const {
  ...
  if (chargedClusterContact.GetConeFraction1() > m_coneEvidenceFraction1) {
    coneEvidence = chargedClusterContact.GetConeFraction1() + chargedClusterContact.GetConeFraction2() +
                   chargedClusterContact.GetConeFraction3();

    if (PandoraContentApi::GetGeometry(*this)->GetHitTypeGranularity(
            chargedClusterContact.GetDaughterCluster()->GetInnerLayerHitType()) <= FINE)
  ...
  if (chargedClusterContact.GetClosestDistanceToHelix() < m_closestTrackEvidence1) {
    trackExtrapolationEvidence =
        (m_closestTrackEvidence1 - chargedClusterContact.GetClosestDistanceToHelix()) / m_closestTrackEvidence1d;

    if (chargedClusterContact.GetClosestDistanceToHelix() < m_closestTrackEvidence2)
      trackExtrapolationEvidence +=
          (m_closestTrackEvidence2 - chargedClusterContact.GetClosestDistanceToHelix()) / m_closestTrackEvidence2d;

    trackExtrapolationEvidence +=
        (m_meanTrackEvidence1 - chargedClusterContact.GetMeanDistanceToHelix()) / m_meanTrackEvidence1d;

    if (chargedClusterContact.GetMeanDistanceToHelix() < m_meanTrackEvidence2)
      trackExtrapolationEvidence +=
          (m_meanTrackEvidence2 - chargedClusterContact.GetMeanDistanceToHelix()) / m_meanTrackEvidence2d;
  }
...
float MainFragmentRemovalAlgorithm::GetRequiredEvidenceForMerge(const Cluster* const pDaughterCluster,
  ...
  const float parentCorrectedClusterEnergy(
      chargedClusterContact.GetParentCluster()->GetTrackComparisonEnergy(this->GetPandora()));
  const float parentTrackEnergy(chargedClusterContact.GetParentTrackEnergy());

  const float oldChi(ReclusterHelper::GetTrackClusterCompatibility(this->GetPandora(), parentCorrectedClusterEnergy,
                                                                   parentTrackEnergy));
  const float newChi(ReclusterHelper::GetTrackClusterCompatibility(
      this->GetPandora(), daughterCorrectedClusterEnergy + parentCorrectedClusterEnergy, parentTrackEnergy));
  ...
  const bool usingGlobalChi2(((newChi2 > oldChi2) && (newChi2 > m_maxGlobalChi2)) ||
                             (globalChi2Evidence < chi2Evidence));
  ...
  const float radialDirectionCosine(pDaughterCluster->GetFitToAllHitsResult().IsFitSuccessful()
                                        ? pDaughterCluster->GetFitToAllHitsResult().GetRadialDirectionCosine()
                                        : 0.f);

  if (radialDirectionCosine < m_angularCorrectionOffset)
    angularCorrection =
        m_angularCorrectionConstant + (radialDirectionCosine - m_angularCorrectionOffset) * m_angularCorrectionGradient;
  ...
  if (pDaughterCluster->PassPhotonId(this->GetPandora())) {
    ...
    if (daughterClusterEnergy < m_photonCorrectionEnergy1 && showerStart < m_photonCorrectionShowerStart2 &&
        showerDiscrepancy < m_photonCorrectionShowerDiscrepancy1)
      photonCorrection = m_photonCorrection4;

    if (daughterClusterEnergy < m_photonCorrectionEnergy1 && showerStart > m_photonCorrectionShowerStart2)
      photonCorrection = m_photonCorrection5;

    if (daughterClusterEnergy < m_photonCorrectionEnergy2 &&
        (showerStart > m_photonCorrectionShowerStart2 || showerDiscrepancy > m_photonCorrectionShowerDiscrepancy2))
  ...
  const float requiredEvidence(usingGlobalChi2 ? globalChi2Evidence + layerCorrection + angularCorrection +
                                                     energyCorrection + leavingCorrection + photonCorrection
                                               : chi2Evidence + layerCorrection + angularCorrection + energyCorrection +
                                                     leavingCorrection + photonCorrection + lowEnergyCorrection);
...
unsigned int MainFragmentRemovalAlgorithm::GetClusterCorrectionLayer(const Cluster* const pDaughterCluster) const {
  ...
  for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(), iterEnd = orderedCaloHitList.end();
    ...
    if ((++layerCounter >= m_correctionLayerNHitLayers) ||
        (energySum > m_correctionLayerEnergyFraction * totalClusterEnergy)) {
      return iter->first;
    }
  }
...
StatusCode MainFragmentRemovalAlgorithm::GetAffectedClusters(const ChargedClusterContactMap& chargedClusterContactMap,
  ...
  for (ChargedClusterContactMap::const_iterator iterI = chargedClusterContactMap.begin(),
                                                iterIEnd = chargedClusterContactMap.end();
    ...
    if (iterI->first == pBestDaughterCluster) {
      for (ChargedClusterContactVector::const_iterator iterJ = iterI->second.begin(), iterJEnd = iterI->second.end();
...
ChargedClusterContact::ChargedClusterContact(const Pandora& pandora, const Cluster* const pDaughterCluster,
...
void ChargedClusterContact::ClusterHelixComparison(const Pandora& pandora, const Cluster* const pDaughterCluster,
                                                   const Cluster* const pParentCluster, const Parameters& parameters) {
  // Configure range of layers in which daughter cluster will be compared to helix fits
  const bool passMipFractionCut(pParentCluster->GetMipFraction() - parameters.m_helixComparisonMipFractionCut >
                                std::numeric_limits<float>::epsilon());

  const unsigned int startLayer(pDaughterCluster->GetInnerPseudoLayer());
  const unsigned int endLayer(
      passMipFractionCut ? std::max(startLayer + parameters.m_helixComparisonStartOffset,
                                    pParentCluster->GetOuterPseudoLayer() + parameters.m_helixComparisonStartOffsetMip)
  ...
  const unsigned int maxOccupiedLayers(passMipFractionCut ? std::numeric_limits<unsigned int>::max()
                                                          : parameters.m_nHelixComparisonLayers);
  ...
  for (TrackList::const_iterator iter = parentTrackList.begin(), iterEnd = parentTrackList.end(); iter != iterEnd;
       ++iter) {
    const Track* const pTrack(*iter);

    // Extract track information
    trackEnergySum += pTrack->GetEnergyAtDca();
    const Helix helix(pTrack->GetTrackStateAtCalorimeter().GetPosition(),
    ...
    const unsigned int nLayersCrossed(
        FragmentRemovalHelper::GetNLayersCrossed(pandora, helix, trackCalorimeterZPosition, clusterZPosition));

    if (nLayersCrossed > parameters.m_maxLayersCrossedByHelix)
      continue;

    // Calculate distance to helix
    float meanDistanceToHelix(std::numeric_limits<float>::max()),
        closestDistanceToHelix(std::numeric_limits<float>::max());

    PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS, !=,
        FragmentRemovalHelper::GetClusterHelixDistance(pDaughterCluster, helix, startLayer, endLayer, maxOccupiedLayers,
                                                       closestDistanceToHelix, meanDistanceToHelix));
...
StatusCode MainFragmentRemovalAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "HelixComparisonMipFractionCut",
                                                       m_contactParameters.m_helixComparisonMipFractionCut));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "HelixComparisonStartOffset", m_contactParameters.m_helixComparisonStartOffset));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "HelixComparisonStartOffsetMip",
                                                       m_contactParameters.m_helixComparisonStartOffsetMip));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "NHelixComparisonLayers", m_contactParameters.m_nHelixComparisonLayers));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxLayersCrossedByHelix", m_contactParameters.m_maxLayersCrossedByHelix));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxTrackClusterDeltaZ", m_contactParameters.m_maxTrackClusterDeltaZ));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ContactCutMeanDistanceToHelix", m_contactCutMeanDistanceToHelix));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ContactCutClosestDistanceToHelix", m_contactCutClosestDistanceToHelix));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ContactCutMaxHitDistance", m_contactCutMaxHitDistance));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "ContactCutMinDaughterInnerLayer", m_contactCutMinDaughterInnerLayer));

  // Track-cluster consistency Chi2 values
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxChi2", m_maxChi2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxGlobalChi2", m_maxGlobalChi2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "Chi2Base", m_chi2Base));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "GlobalChi2Penalty", m_globalChi2Penalty));

  // Correction layer parameters
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "CorrectionLayerNHitLayers", m_correctionLayerNHitLayers));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "CorrectionLayerEnergyFraction", m_correctionLayerEnergyFraction));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ClosestTrackEvidence1", m_closestTrackEvidence1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ClosestTrackEvidence1d", m_closestTrackEvidence1d));

  if (m_closestTrackEvidence1d < std::numeric_limits<float>::epsilon())
    return STATUS_CODE_INVALID_PARAMETER;

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ClosestTrackEvidence2", m_closestTrackEvidence2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "ClosestTrackEvidence2d", m_closestTrackEvidence2d));

  if (m_closestTrackEvidence2d < std::numeric_limits<float>::epsilon())
    return STATUS_CODE_INVALID_PARAMETER;

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MeanTrackEvidence1", m_meanTrackEvidence1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MeanTrackEvidence1d", m_meanTrackEvidence1d));

  if (m_meanTrackEvidence1d < std::numeric_limits<float>::epsilon())
    return STATUS_CODE_INVALID_PARAMETER;

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MeanTrackEvidence2", m_meanTrackEvidence2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MeanTrackEvidence2d", m_meanTrackEvidence2d));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "TrackExtrapolationWeight", m_trackExtrapolationWeight));

  // Required evidence: Layer correction
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "LayerCorrectionLayerValue1", m_layerCorrectionLayerValue1));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "LayerCorrectionLayerValue2", m_layerCorrectionLayerValue2));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "LayerCorrectionLayerValue3", m_layerCorrectionLayerValue3));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "LayerCorrection1", m_layerCorrection1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "LayerCorrection2", m_layerCorrection2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "LayerCorrection3", m_layerCorrection3));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "LayerCorrection4", m_layerCorrection4));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "LayerCorrectionLayerSpan", m_layerCorrectionLayerSpan));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "LayerCorrectionMinInnerLayer", m_layerCorrectionMinInnerLayer));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "LayerCorrection5", m_layerCorrection5));

  // Leaving correction
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "LeavingCorrection", m_leavingCorrection));

  // Energy correction
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "EnergyCorrectionThreshold", m_energyCorrectionThreshold));

  // Low energy correction
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "LowEnergyCorrectionThreshold", m_lowEnergyCorrectionThreshold));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "LowEnergyCorrectionNHitLayers1", m_lowEnergyCorrectionNHitLayers1));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "LowEnergyCorrectionNHitLayers2", m_lowEnergyCorrectionNHitLayers2));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "LowEnergyCorrectionNHitLayers3", m_lowEnergyCorrectionNHitLayers3));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "LowEnergyCorrection1", m_lowEnergyCorrection1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "LowEnergyCorrection2", m_lowEnergyCorrection2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "LowEnergyCorrection3", m_lowEnergyCorrection3));

  // Angular correction
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "AngularCorrectionOffset", m_angularCorrectionOffset));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "AngularCorrectionConstant", m_angularCorrectionConstant));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "AngularCorrectionGradient", m_angularCorrectionGradient));

  // Photon correction
  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "PhotonCorrectionEnergy1", m_photonCorrectionEnergy1));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "PhotonCorrectionEnergy2", m_photonCorrectionEnergy2));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "PhotonCorrectionEnergy3", m_photonCorrectionEnergy3));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "PhotonCorrectionShowerStart1", m_photonCorrectionShowerStart1));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "PhotonCorrectionShowerStart2", m_photonCorrectionShowerStart2));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "PhotonCorrectionShowerDiscrepancy1", m_photonCorrectionShowerDiscrepancy1));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "PhotonCorrectionShowerDiscrepancy2", m_photonCorrectionShowerDiscrepancy2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PhotonCorrection1", m_photonCorrection1));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PhotonCorrection2", m_photonCorrection2));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PhotonCorrection3", m_photonCorrection3));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PhotonCorrection4", m_photonCorrection4));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PhotonCorrection5", m_photonCorrection5));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PhotonCorrection6", m_photonCorrection6));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "PhotonCorrection7", m_photonCorrection7));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinRequiredEvidence", m_minRequiredEvidence));
