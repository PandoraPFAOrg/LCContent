...
StatusCode ProximityBasedMergingAlgorithm::Run() {
  ...
  // Examine pairs of clusters to evaluate merging suitability. Begin by comparing clusters in highest layers with those
  // in lowest layers.
  for (ClusterVector::reverse_iterator iterI = clusterVector.rbegin(), iterIEnd = clusterVector.rend();
    ...
    if (!ClusterHelper::CanMergeCluster(this->GetPandora(), pDaughterCluster, m_canMergeMinMipFraction,
                                        m_canMergeMaxRms))
      continue;
    ...
    for (ClusterVector::const_iterator iterJ = clusterVector.begin(), iterJEnd = clusterVector.end(); iterJ != iterJEnd;
         ++iterJ) {
      ...
      if (!ClusterHelper::CanMergeCluster(this->GetPandora(), pParentCluster, m_canMergeMinMipFraction,
                                          m_canMergeMaxRms))
        continue;
      ...
      const int showerLayerSpan(static_cast<int>(daughterInnerLayer) -
                                static_cast<int>(pParentCluster->GetShowerStartLayer(this->GetPandora())));
      ...
      for (TrackList::const_iterator trackIter = trackList.begin(), trackIterEnd = trackList.end();
      ...
      if (STATUS_CODE_SUCCESS != this->GetGenericDistanceBetweenClusters(pParentCluster, pDaughterCluster, startLayer,
                                                                         endLayer, genericDistance))
        continue;
    ...
    const CartesianVector parentInnerLayerCentroid(
        pBestParentCluster->GetCentroid(pBestParentCluster->GetInnerPseudoLayer()));
    ...
    if (this->IsClusterFragment(pBestParentCluster, pDaughterCluster)) {
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               PandoraContentApi::MergeAndDeleteClusters(*this, pBestParentCluster, pDaughterCluster));
      *iterI = NULL;
    }
  }
...
StatusCode ProximityBasedMergingAlgorithm::GetGenericDistanceBetweenClusters(const Cluster* const pParentCluster,
                                                                             const Cluster* const pDaughterCluster,
  ...
  for (unsigned int iLayer = startLayer; iLayer <= endLayer; ++iLayer) {
    ...
    for (CaloHitList::const_iterator hitIterP = iterP->second->begin(), hitIterPEnd = iterP->second->end();
      ...
      const unsigned int firstExaminationLayer(
          (iLayer > m_nAdjacentLayersToExamine) ? iLayer - m_nAdjacentLayersToExamine : 0);
      const unsigned int lastExaminationLayer(iLayer + m_nAdjacentLayersToExamine);

      for (unsigned int iExaminationLayer = firstExaminationLayer; iExaminationLayer <= lastExaminationLayer;
           ++iExaminationLayer) {
        ...
        for (CaloHitList::const_iterator hitIterD = iterD->second->begin(), hitIterDEnd = iterD->second->end();
...
bool ProximityBasedMergingAlgorithm::IsClusterFragment(const Cluster* const pParentCluster,
                                                       const Cluster* const pDaughterCluster) const {
  ...
  const float closeHitFraction(
      FragmentRemovalHelper::GetFractionOfCloseHits(pDaughterCluster, pParentCluster, m_closeHitThreshold));
  ...
  StatusCode statusCode = FragmentRemovalHelper::GetClusterContactDetails(
      pDaughterCluster, pParentCluster, m_clusterContactThreshold, nContactLayers, contactFraction);

  if ((STATUS_CODE_SUCCESS == statusCode) &&
      ((contactFraction - m_minContactFraction) > std::numeric_limits<float>::epsilon())) {
    return true;
  }
  ...
  for (TrackList::const_iterator trackIter = parentTrackList.begin(), trackIterEnd = parentTrackList.end();
       trackIter != trackIterEnd; ++trackIter) {
    ...
    if ((std::fabs(momentum.GetZ()) < std::numeric_limits<float>::epsilon()) ||
        ((momentum.GetMagnitude() / momentum.GetZ()) * deltaZ > m_maxHelixPathlengthToDaughter)) {
      continue;
    }
    ...
    if (STATUS_CODE_SUCCESS != FragmentRemovalHelper::GetClusterHelixDistance(
                                   pDaughterCluster, helix, daughterInnerLayer,
                                   daughterInnerLayer + m_helixDistanceNLayers, m_helixDistanceMaxOccupiedLayers,
                                   closestDistanceToHit, meanDistanceToHits)) {
      continue;
    }
...
StatusCode ProximityBasedMergingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                           XmlHelper::ProcessFirstAlgorithm(*this, xmlHandle, m_trackClusterAssociationAlgName));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "CanMergeMinMipFraction", m_canMergeMinMipFraction));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "CanMergeMaxRms", m_canMergeMaxRms));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinClusterInnerLayer", m_minClusterInnerLayer));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinLayerSpan", m_minLayerSpan));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinShowerLayerSpan", m_minShowerLayerSpan));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxTrackClusterChi", m_maxTrackClusterChi));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      XmlHelper::ReadValue(xmlHandle, "NAdjacentLayersToExamine", m_nAdjacentLayersToExamine));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MaxParallelDistance", m_maxParallelDistance));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxInnerLayerSeparation", m_maxInnerLayerSeparation));
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinContactFraction", m_minContactFraction));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "CloseHitThreshold", m_closeHitThreshold));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinCloseHitFraction", m_minCloseHitFraction));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxHelixPathlengthToDaughter", m_maxHelixPathlengthToDaughter));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "HelixDistanceNLayers", m_helixDistanceNLayers));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "HelixDistanceMaxOccupiedLayers", m_helixDistanceMaxOccupiedLayers));

  PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
      XmlHelper::ReadValue(xmlHandle, "MaxClusterHelixDistance", m_maxClusterHelixDistance));
