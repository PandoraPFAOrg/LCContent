...
BIBHitMergingAlgorithm::BIBHitMergingAlgorithm() :
    ...
    m_minHostClusterEnergy(2.f),
    m_maxRecombinationDistance(50.f),
    m_minClusterNeighbours(2),
    m_neighbourNLayers(2),
    m_neighbourCutDistanceSquared(25.f * 25.f),
    m_neighbourMaxSeparationSquared(1000.f * 1000.f),
    m_searchSafetyFactor(2.5f),
    m_layerSpanTolerance(2),
    m_timingCutEnabled(false),
    m_maxTimeDifference(1.f)
{
}
...
StatusCode BIBHitMergingAlgorithm::Run()
{
    ...
    for (ClusterList::const_iterator iterJ = clusterList.begin(), iterJEnd = clusterList.end(); iterJ != iterJEnd; ++iterJ)
    {
        ...
        for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(), iterEnd = orderedCaloHitList.end(); iter != iterEnd; ++iter)
        {
            for (CaloHitList::const_iterator hitIter = iter->second->begin(), hitIterEnd = iter->second->end(); hitIter != hitIterEnd; ++hitIter)
            {
                ...
                {
                    minpos[0] = position.GetX(); minpos[1] = position.GetY(); minpos[2] = position.GetZ();
                    maxpos[0] = position.GetX(); maxpos[1] = position.GetY(); maxpos[2] = position.GetZ();
                }
                else
                {
    ...
    for (CaloHitList::const_iterator hitIterI = pCaloHitList->begin(); hitIterI != pCaloHitList->end(); ++hitIterI)
    {
        ...
        for (std::vector<HitKDNode3D>::const_iterator iterF = found.begin(), iterFEnd = found.end(); iterF != iterFEnd; ++iterF)
        {
            ...
            const unsigned int layerDifference((memberLayer > pseudoLayer) ? memberLayer - pseudoLayer : pseudoLayer - memberLayer);
        ...
        for (std::map<unsigned int, unsigned int>::const_iterator iterC = neighbourCounts.begin(), iterCEnd = neighbourCounts.end(); iterC != iterCEnd; ++iterC)
        {
...
StatusCode BIBHitMergingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
{
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, XmlHelper::ReadValue(xmlHandle,
        "MinHostClusterEnergy", m_minHostClusterEnergy));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, XmlHelper::ReadValue(xmlHandle,
        "MaxRecombinationDistance", m_maxRecombinationDistance));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, XmlHelper::ReadValue(xmlHandle,
        "MinClusterNeighbours", m_minClusterNeighbours));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, XmlHelper::ReadValue(xmlHandle,
        "NeighbourNLayers", m_neighbourNLayers));

    float neighbourCutDistance(std::sqrt(m_neighbourCutDistanceSquared));
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, XmlHelper::ReadValue(xmlHandle,
        "NeighbourCutDistance", neighbourCutDistance));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, XmlHelper::ReadValue(xmlHandle,
        "NeighbourMaxSeparation", neighbourMaxSeparation));
    m_neighbourMaxSeparationSquared = neighbourMaxSeparation * neighbourMaxSeparation;

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, XmlHelper::ReadValue(xmlHandle,
        "SearchSafetyFactor", m_searchSafetyFactor));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, XmlHelper::ReadValue(xmlHandle,
        "LayerSpanTolerance", m_layerSpanTolerance));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, XmlHelper::ReadValue(xmlHandle,
        "TimingCutEnabled", m_timingCutEnabled));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, XmlHelper::ReadValue(xmlHandle,
        "MaxTimeDifference", m_maxTimeDifference));
