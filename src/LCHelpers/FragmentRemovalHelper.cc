...
float FragmentRemovalHelper::GetFractionOfCloseHits(const Cluster* const pClusterI, const Cluster* const pClusterJ,
  ...
  for (OrderedCaloHitList::const_iterator iterI = orderedCaloHitListI.begin(), iterIEnd = orderedCaloHitListI.end();
       iterI != iterIEnd; ++iterI) {
    for (CaloHitList::const_iterator hitIterI = iterI->second->begin(), hitIterIEnd = iterI->second->end();
         hitIterI != hitIterIEnd; ++hitIterI) {
      ...
      for (OrderedCaloHitList::const_iterator iterJ = orderedCaloHitListJ.begin(), iterJEnd = orderedCaloHitListJ.end();
           iterJ != iterJEnd; ++iterJ) {
        for (CaloHitList::const_iterator hitIterJ = iterJ->second->begin(), hitIterJEnd = iterJ->second->end();
             hitIterJ != hitIterJEnd; ++hitIterJ) {
...
float FragmentRemovalHelper::GetFractionOfHitsInCone(const Pandora& pandora, const Cluster* const pClusterI,
                                                     const Cluster* const pClusterJ, const float coneCosineHalfAngle) {
...
float FragmentRemovalHelper::GetFractionOfHitsInCone(const Cluster* const pCluster, const Track* const pTrack,
...
float FragmentRemovalHelper::GetFractionOfHitsInCone(const Cluster* const pCluster, const CartesianVector& coneApex,
                                                     const CartesianVector& coneDirection,
                                                     const float coneCosineHalfAngle) {
...
StatusCode FragmentRemovalHelper::GetClusterHelixDistance(const Cluster* const pCluster, const Helix& helix,
                                                          const unsigned int startLayer, const unsigned int endLayer,
                                                          const unsigned int maxOccupiedLayers,
                                                          float& closestDistanceToHit, float& meanDistanceToHits) {
  ...
  for (unsigned int iLayer = startLayer; iLayer <= endLayer; ++iLayer) {
    ...
    for (CaloHitList::const_iterator hitIter = iter->second->begin(), hitIterEnd = iter->second->end();
      ...
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                               helix.GetDistanceToPoint((*hitIter)->GetPositionVector(), distanceVector));
...
StatusCode FragmentRemovalHelper::GetClusterContactDetails(const Cluster* const pClusterI,
                                                           const Cluster* const pClusterJ,
                                                           const float distanceThreshold, unsigned int& nContactLayers,
  ...
  for (unsigned int iLayer = startLayer; iLayer <= endLayer; ++iLayer) {
    OrderedCaloHitList::const_iterator iterI = orderedCaloHitListI.find(iLayer),
                                       iterJ = orderedCaloHitListJ.find(iLayer);
...
float FragmentRemovalHelper::GetEMEnergyWeightedLayerSeparation(const Cluster* const pClusterI,
                                                                const Cluster* const pClusterJ) {
  ...
  for (unsigned int pseudoLayer = startPseudoLayer; pseudoLayer <= endPseudoLayer; ++pseudoLayer) {
    ...
    for (CaloHitList::const_iterator hIterJ = iterJ->second->begin(), hIterJEnd = iterJ->second->end();
         hIterJ != hIterJEnd; ++hIterJ) {
      const CaloHit* const pCaloHitJ(*hIterJ);
      const CartesianVector& positionJ(pCaloHitJ->GetPositionVector());

      for (CaloHitList::const_iterator hIterI = iterI->second->begin(), hIterIEnd = iterI->second->end();
           hIterI != hIterIEnd; ++hIterI) {
...
CartesianVector FragmentRemovalHelper::GetEMEnergyWeightedPosition(const Cluster* const pCluster) {
  ...
  for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(), iterEnd = orderedCaloHitList.end();
       iter != iterEnd; ++iter) {
    for (CaloHitList::const_iterator hIter = iter->second->begin(), hIterEnd = iter->second->end(); hIter != hIterEnd;
         ++hIter) {
...
ClusterContact::ClusterContact(const Pandora& pandora, const Cluster* const pDaughterCluster,
                               const Cluster* const pParentCluster, const Parameters& parameters,
                               ClusterContactCache& contactCache)
    : m_pDaughterCluster(pDaughterCluster), m_pParentCluster(pParentCluster), m_nContactLayers(0),
      m_contactFraction(0.f), m_coneFraction1(FragmentRemovalHelper::GetFractionOfHitsInCone(
                                  pandora, pDaughterCluster, pParentCluster, parameters.m_coneCosineHalfAngle1)),
      m_closeHitFraction1(0.f), m_closeHitFraction2(0.f), m_distanceToClosestHit(std::numeric_limits<float>::max()) {
  (void)FragmentRemovalHelper::GetClusterContactDetails(
      pDaughterCluster, pParentCluster, parameters.m_distanceThreshold, m_nContactLayers, m_contactFraction);
  ...
  for (const auto& layerEntryI : orderedCaloHitListI) {
    for (const CaloHit* const pCaloHitI : *(layerEntryI.second)) {
      ...
      for (const auto& layerEntryJ : orderedCaloHitListJ) {
        ...
        if (layerBoundingBoxJ.IsSeparatedFromSquared(positionVectorI,
                                                     std::max(minDistanceSquared, closeHitDistanceSquared)))
          continue;
